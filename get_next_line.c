...
char    *read_file(int fd, char *res)/* on creer une fonction avec un fd, et une chaine de caractere */
{
    ...
    {
        // while not eof read
        byte_read = read(fd, buffer, BUFFER_SIZE);/* alors byte read  lis la chaine, de taille buffer size sur lentree fd,*/
        ...
        // join and free
        res = ft_free(res, buffer);/* notre chaine res vaux ft*/
        // quit if \n find
        if (ft_strchr(buffer, '\n'))/* */
            break ;/* */
    }
